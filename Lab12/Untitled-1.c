   ...
   // You will need this code
	while (fgets (string, 256, file))     {
		...
		if(0==nc||0==nr){
			...
			nc = atof(pch); //will just get nc
			pch = strtok (NULL, "  "); //makes it so that pch is just nr
			if(pch == NULL){
				fprintf(stderr,"didn't find nr");
				exit(99);
			}
			nr = atof(pch);
			if(nc<=0||nr<=0){
				fprintf(stderr,"can't work w/ negatives");
			}
			M = gsl_matrix_calloc(nc, nr);
			return M;
		}
    }
	return M;
...
//   for (int j = 0; j < GSL_MAG_COL(M); j++){
	for(row =0; row<GSL_MAT_ROWS(M);row++){
		printf("\nrow : %d\n",row);
		// while (fgets (string, 256, file))  {//if the buffer length is too small make more buffer

		while ( (strlen(fgets (string, 256, file)))>bufferLength)  {//if the buffer length is too small make more buffer
				printf("\nsize: %lf\n",x);
				printf("\nstring ");
				printf(string);
				...
				bufferLength *=2;
				string =(char*)realloc(string,sizeof(char)*bufferLength);
				if(string==NULL){
					fprintf(stderr,"didn't assign data");
					exit(99);
				}
			...
			if(feof(file)){
				fprintf(stderr,"END OF FILE ERR\n not enough rows in data\n");
				exit(-99);
			}
			printf("\nstring: ");
			printf(string);
			...
			data = strtok (string, "    "); 
			if(data == NULL){
				fprintf(stderr,"didn't split data");
				exit(99);
			}
			...
			//TODO
			//should be able to read B vector Data
			data= strtok(NULL," ");
			if(data == NULL){ 
			...
			data= strtok(NULL," ");
			if(data == NULL){
				fprintf(stderr,"TOO MANY COLLUMS");
				exit(99);
			}
